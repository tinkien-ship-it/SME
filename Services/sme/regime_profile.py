...
def get_ledger_profile(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        # DN siêu nhỏ thường kê khai GTGT theo quý
        'default_vat_filing_mode': 'quarterly' if is_tt58 else 'monthly',
        'label': 'TT58 (siêu nhỏ)' if is_tt58 else 'TT99 (vừa và nhỏ)',
        'bctc_hint': (
            'TT58: vẫn dùng sổ kép TT99 trong phần mềm; tờ khai/NSNN mặc định theo quý.'
            if is_tt58 else
            'TT99: sổ kép đầy đủ · GTGT mặc định theo tháng (có thể đổi quý).'
        ),
    }
